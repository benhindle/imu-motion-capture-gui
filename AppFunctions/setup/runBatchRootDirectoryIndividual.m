function runBatchRootDirectoryIndividual(app)
...
app.debugTitle = "";
 ...
 dateFolderNames = {app.dateId.Value};
...
for date = 1 : length(dateFolderNames)
    dateDir = fullfile(app.rootDirectoryPath, app.dateId.Value);
    participantFolderNames = {app.participantId.Value};
...
%         directoryFileList = dir(fullfile(participantDir, "*.csv"));
        ...
        directoryFileList = app.trialFiles;
        if ischar(app.trialFiles)
            directoryFileList = [convertCharsToStrings(app.trialFiles)];
        end
        
        participantFileCount = 0;
        for fileIndex= 1 : length(directoryFileList)
            if ~isequal(directoryFileList(fileIndex), staticFileList(1).name) && ~isequal(directoryFileList(fileIndex), magFileList(1).name)
                participantFileCount = participantFileCount + 1;
            end
        end
        
        for file = 1 : length(directoryFileList)
            filePath = fullfile(participantDir, directoryFileList(file));
            app.trialPath = participantDir;
            
            if ~isequal(directoryFileList(file), staticFileList(1).name) && ~isequal(directoryFileList(file), magFileList(1).name)
...
%                 fprintf("date: %s\n", app.dateId.Value);
                ...
                app.tempTrialFile = strcat(filesep, directoryFileList(file));
                tableOut = table;
                ...
                for j = 1:length(app.joints)
                    ...
                    app.plotfiletitle = split(directoryFileList(file), ".csv");
                ...
                exportfiletitle = split(directoryFileList(file), ".csv");
